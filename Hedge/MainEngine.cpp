...
uint32_t MainEngine::loadWeaponsEntity(const LevelManager &levelManager)
{
    ...
    //+1 for grenades
    audioComp->m_soundElements.resize(vectWeapons.size() + 1);
    ...
    loadGrenadesData(levelManager, *weaponConf, *audioComp);
    return weaponEntity;
}

//===================================================================
void MainEngine::loadGrenadesData(const LevelManager &levelManager, WeaponComponent &weaponConf, AudioComponent &audioComp)
{
    const WeaponINIData &grenadesData = levelManager.getGrenadesData();
    weaponConf.m_grenadeData.m_ammunationsCount = *grenadesData.m_startingAmmoCount;
    weaponConf.m_grenadeData.m_maxAmmunations = grenadesData.m_maxAmmo;
    weaponConf.m_grenadeData.m_weaponPower = grenadesData.m_damage;
...
bool MainEngine::loadEnemiesEntities(const LevelManager &levelManager)
{
    ...
    for(std::map<std::string, EnemyData>::const_iterator it = enemiesData.begin(); it != enemiesData.end(); ++it)
    {
        ...
        const SpriteData &memSpriteData = levelManager.getPictureData().getSpriteData()[it->second.m_staticLeftSprites[0]];
                                                           ...
                                                           bool moveable, uint32_t shapeNum, bool loadFromCheckpoint)
{
         ...
         it != currentShape.second.m_TileGamePosition.end(); ++it, ++wallNum)
    {
        ...
        confBaseWallData(numEntity, memSpriteData, moveableWallCorrectedPos, moveable, collShape, vectSprite, currentShape.second);
        if(!moveable)
        {
            continue;
        }
...
//===================================================================
void MainEngine::confBaseWallData(uint32_t wallEntity, const SpriteData &memSpriteData, const PairUI_t &coordLevel, bool moveable,
                                  CollisionShape_e collShape, const std::vector<SpriteData> &vectSprite, const MoveableWallData &wallData)
{
    CollisionTag_e tag = wallData.m_traversable ? CollisionTag_e::TRAVERSABLE_WALL_CT : CollisionTag_e::WALL_CT;
    ...
    if(moveable && wallData.m_triggerBehaviourType != TriggerBehaviourType_e::AUTO &&
...
//===================================================================
uint32_t MainEngine::createGrenadeEntity()
{
    std::array<uint32_t, Components_e::TOTAL_COMPONENTS> vect;
    vect.fill(0);
    vect[Components_e::AUDIO_COMPONENT] = 1;
...
//===================================================================
void MainEngine::playerThrowGrenade()
{
    PlayerConfComponent *playerConf = Ecsm_t::instance().getComponent<PlayerConfComponent, Components_e::PLAYER_CONF_COMPONENT>(m_playerEntity);
    assert(playerConf);
    ...
    uint32_t grenadeEntity;
    for(uint32_t i = 0; i < weaponComp->m_grenadeData.m_visibleShootEntities->size(); ++i)
    {
        grenadeEntity = (*weaponComp->m_grenadeData.m_visibleShootEntities)[i];
        GeneralCollisionComponent *collComp = Ecsm_t::instance().getComponent<GeneralCollisionComponent, Components_e::GENERAL_COLLISION_COMPONENT>(grenadeEntity);
        assert(collComp);
        ...
        mapComp->m_absoluteMapPositionPX = mapPlayerComp->m_absoluteMapPositionPX;
...
void MainEngine::updateConfirmLoadingMenuInfo(PlayerConfComponent &playerComp)
{
    ...
    else if(playerComp.m_menuMode == MenuMode_e::CONFIRM_QUIT_INPUT_FORM)
    {
        ...
        writeComp->addTextLine({{}, "Sauvegarder Les Changements?"});
    }
            ...
            playerComp.m_menuMode == MenuMode_e::CONFIRM_QUIT_GAME)
    {
        ...
        if(playerComp.m_menuMode == MenuMode_e::CONFIRM_LOADING_GAME_FORM)
        {
            ...
            {
                writeComp->addTextLine({{}, "La Sauvegarde Precedente Sera Ecrasee"});
            }
            if(!writeComp->m_vectMessage.empty())
            {
                writeComp->addTextLine({{}, "Continuer?"});
            }
            //TITLE MENU CASE
            else
            {
                ...
                {
                    writeComp->addTextLine({{}, "Commencer Nouveau Jeu?"});
                }
        ...
        {
            writeComp->addTextLine({{}, "Quitter Le Jeu?"});
        }
    }
                                   ...
                                   PlayerConfComponent &playerComp)
{
    ...
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_aimUpLookRight, PlayerSpriteElementType_e::AIM_UP_LOOK_RIGHT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_aimUpLookLeft, PlayerSpriteElementType_e::AIM_UP_LOOK_LEFT);

    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_aimUpRight, PlayerSpriteElementType_e::AIM_UP_RIGHT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_aimDownRight, PlayerSpriteElementType_e::AIM_DOWN_RIGHT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_aimUpLeft, PlayerSpriteElementType_e::AIM_UP_LEFT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
    ...
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_runAimDownLeft, PlayerSpriteElementType_e::RUN_AIM_DOWN_LEFT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_jumpAimUpLookRight, PlayerSpriteElementType_e::JUMP_AIM_UP_LOOK_RIGHT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_jumpAimUpRight, PlayerSpriteElementType_e::JUMP_AIM_UP_RIGHT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_jumpAimDownRight, PlayerSpriteElementType_e::JUMP_AIM_DOWN_RIGHT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_jumpAimDownLookRight, PlayerSpriteElementType_e::JUMP_AIM_DOWN_LOOK_RIGHT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_jumpAimUpLookLeft, PlayerSpriteElementType_e::JUMP_AIM_UP_LOOK_LEFT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_jumpAimUpLeft, PlayerSpriteElementType_e::JUMP_AIM_UP_LEFT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_jumpAimDownLeft, PlayerSpriteElementType_e::JUMP_AIM_DOWN_LEFT);
    insertPlayerSpriteFromType(vectSprite, playerComp.m_mapSpriteAssociate, memSpriteComp->m_vectSpriteData,
                               playerData.m_jumpAimDownLookLeft, PlayerSpriteElementType_e::JUMP_AIM_DOWN_LOOK_LEFT);
}
...
//===================================================================
uint32_t MainEngine::createVehiculeEntity()
{
    std::array<uint32_t, Components_e::TOTAL_COMPONENTS> vect;
    vect.fill(0);
    vect[Components_e::POSITION_VERTEX_COMPONENT] = 1;
                                  ...
                                  uint32_t numWeaponEntity, uint32_t numDisplayTeleportEntity)
{
    ...
    rectColl->m_offset = {rectColl->m_size.first / 1.5f, rectColl->m_size.second};
...
//===================================================================
void MainEngine::confPlayerVisibleShotsSprite(const std::vector<SpriteData> &vectSpriteData, const MapVisibleShotData_t &shootDisplayData, WeaponComponent &weaponComp)
{
    for(uint32_t i = 0; i < weaponComp.m_weaponsData.size(); ++i)
    {
        if(weaponComp.m_weaponsData[i].m_attackType == AttackType_e::VISIBLE_SHOTS)
        {
            weaponComp.m_weaponsData[i].m_visibleShootEntities = std::vector<uint32_t>();
            (*weaponComp.m_weaponsData[i].m_visibleShootEntities).resize(1);
...
bool MainEngine::loadStaticElementEntities(const LevelManager &levelManager)
{
    ...
    loadStaticElementGroup(vectSprite, levelManager.getVehiculesData(), LevelStaticElementType_e::VEHICULE, levelManager);
                                                              ...
                                                              const std::vector<SpriteData> &vectSpriteData, uint32_t iterationNum, const LevelManager &levelManager, bool enemyDrop)
{
    ...
    else
    {
        ...
        {
            tag = CollisionTag_e::VEHICULE_CT;
            entityNum = createVehiculeEntity();
        }
        else
        {
...
uint32_t MainEngine::confObjectEntity(const StaticLevelElementData &objectData)
{
    ...
    if(objComp->m_type == ObjectType_e::AMMO_WEAPON || objComp->m_type == ObjectType_e::WEAPON ||
            objComp->m_type == ObjectType_e::HEAL || objComp->m_type == ObjectType_e::GRENADE)
    {
